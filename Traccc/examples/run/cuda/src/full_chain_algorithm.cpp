...
full_chain_algorithm::full_chain_algorithm(
    ...
    : messaging(logger->clone()),
      ...
      m_finding(
          finding_config, {m_cached_device_mr, &m_cached_pinned_host_mr},
          m_copy, m_stream, logger->cloneWithSuffix("TrackFindingAlg"),
          std::make_unique<traccc::cuda::kalman_fitting_algorithm>(
              finding_config.kalman_smoother,
              traccc::memory_resource{m_cached_device_mr,
                                      &m_cached_pinned_host_mr},
              m_copy, m_stream, logger->cloneWithSuffix("KalmanSmootherAlg"))),
...
full_chain_algorithm::full_chain_algorithm(const full_chain_algorithm& parent)
    : messaging(parent.logger().clone()),
      ...
      m_finding(parent.m_finding_config,
                ...
                std::make_unique<traccc::cuda::kalman_fitting_algorithm>(
                    parent.m_finding_config.kalman_smoother,
                    traccc::memory_resource{m_cached_device_mr,
                                            &m_cached_pinned_host_mr},
                    m_copy, m_stream,
                    parent.logger().cloneWithSuffix("KalmanSmootherAlg"))),
